  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      if (session.role !== 'manager') {
        return { statusCode: 403, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: false, error: 'Только менеджер может изменять сделки' }) };
      }
      let body;
      ...
      if (body.id) {
        ...
        if (idx === -1) return { statusCode: 404, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: false, error: 'Сделка не найдена' }) };
